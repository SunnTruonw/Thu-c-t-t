...
// tạo link
function makeLink($type, $id = null, $category = null, $slug = null, $request = [])
{
    $route = "";
    switch ($type) {
        ...
        case 'product':
            if ($slug) {
                $route = route("product.detail", ["category" => $category,"slug" => $slug]);
            } else {
                $route = "#";
            }
            break;
...
function makeLinkToLanguage($type, $id = null, $slug = null, $lang = 'vi', $request = [])
{
    ...
    switch ($type) {
        ...
        case 'collection':
            ...
            break;
		case 'drug-store':
            $route = route("home.drugStore" . $lang);
            break;
        default:
            $route = route("home.index");
            break;
...
function menuRecusiveLimit($model, $id, $result = array(), $i = 0)
{
    //  global $result;
    $i++;
    $data = $model->where('active', 1)->select(['id'])->orderby('order')->orderByDesc('created_at')->find($id);
    ...
    $childs =  $data->childs()->where('active', 1)->select(['id'])->orderby('order')->orderByDesc('created_at')->limit(10)->get();
    foreach ($childs as $child) {
        //  $res  = $child->setAppends(['slug'])->toArray();
