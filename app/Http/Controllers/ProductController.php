...
class ProductController extends Controller
{
    ...
    public function loadComment(Request $request)
    {
        if ($request->ajax()) {
            ...
            $dataComment = $dataProduct->comments()->where('parent_id', 0)
                // ->where('active', 1)
                ->where('type_comment', $request->type_comment)
                ->orderBy('id', 'DESC')
                ->paginate(5);

            $totalItems = $dataProduct->comments()
                // ->where('active', 1)
                ->where('type_comment', $request->type_comment)->count();

            $html = view('frontend.components.load-comment-product', ['data' => $dataComment])->render();
